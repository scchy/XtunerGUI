...
def convert_to_hf(config_file, pth_model, save_dir):
    args = parse_args()

    # parse config
    if not os.path.isfile(config_file):
        try:
            config_file = cfgs_name_path[config_file]
        except KeyError:
    ...
    print(f'Saving HuggingFace model to {save_dir}')
    model.llm.save_pretrained(
        save_dir, max_shard_size=args.max_shard_size)
    if 'PeftModel' not in model.llm.__class__.__name__:
        print(f'Saving HuggingFace tokenizer to {save_dir}')
        tokenizer = BUILDER.build(cfg.tokenizer)
        tokenizer.save_pretrained(save_dir)
    shutil.copyfile(config_file, os.path.join(save_dir,
                                              'xtuner_config.py'))
    print('Pth to hf all done!')
...
if __name__ == '__main__':
    ...
    save_dir = '/root/ft-oasst1/hf5'

    convert_to_hf(config_file, pth_model, save_dir)